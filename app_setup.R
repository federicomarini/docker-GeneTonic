...
if((input_folder == '' || !dir.exists(input_folder))) {
  ...
  app_genetonic <- GeneTonic()
  shiny::runApp(app_genetonic, launch.browser = FALSE, port = 3838, host = "0.0.0.0")
} else {
  gtl_obj_path <- file.path(input_folder, "gtl.RDS")
  
  if(!file.exists(gtl_obj_path)) {
    # run "as server", basically without dataset specified
    app_genetonic <- GeneTonic()
    shiny::runApp(app_genetonic, launch.browser = FALSE, port = 3838, host = "0.0.0.0")
  } else {
    gtl_obj <- readRDS(gtl_obj_path)
    app_genetonic <- GeneTonic(gtl_obj = gtl_obj)
    shiny::runApp(app_genetonic, launch.browser = FALSE, port = 3838, host = "0.0.0.0")
  }
}
