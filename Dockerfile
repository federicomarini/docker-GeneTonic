...
RUN Rscript -e "options(repos = c(CRAN = 'https://cran.r-project.org')); install.packages('BiocManager')" && \
    ...
    Rscript -e "options(repos = c(CRAN = 'https://cran.r-project.org')); options(Ncpus = 4); BiocManager::install('GeneTonic', dependencies=TRUE)" 
  
    # Rscript -e "devtools::install_github('federicomarini/ideal', ref = 'intogalaxy')"

# will probably need to fit into the "official" dependencies...
RUN Rscript -e "options(repos = c(CRAN = 'https://cran.r-project.org')); install.packages('markdown')"
